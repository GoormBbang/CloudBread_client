...
export default function SignUpHealthScreen({ navigation }: any) {
  ...
  if (isLoading) {
    return (
      ...
      >
        ...
        >
          <View className="w-full mb-6 "></View>
  ...
  const dietItems: CheckboxData[] =
    metadata?.result?.dietTypes.map((item) => ({
  ...
  const healthItems: CheckboxData[] =
    metadata?.result?.healthTypes.map((item) => ({
      id: item.id,
      label: item.name,
      ...
      >
        <View className="w-full mb-6 ">
          ...
          <Checkbox
            ...
            items={dietItems}
            onItemPress={(id) =>
              handleCheckboxToggle(id, selectedDietIds, setSelectedDietIds)
            }
            className="w-full"
          />
          <Checkbox
            ...
            items={healthItems}
            onItemPress={(id) =>
              handleCheckboxToggle(id, selectedHealthIds, setSelectedHealthIds)
            }
            className="w-full"
          />
          <MultiSelectFilter
