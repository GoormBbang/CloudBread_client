...
import { View, Text, ScrollView, Image, Alert, ActivityIndicator, TouchableOpacity, KeyboardAvoidingView, Platform } from "react-native";
...
import { getFoodSuggest, postImageUpload, postSelectedFood } from '../../api/services/camera';
...
const handleSelectFood = async() => {
  ...
  return (
    <View style={{ flex: 1 }}>
     ...
     {Array.isArray(candidates) && candidates.map((item,index)=>(
 <View key={index} className='w-full h-fit flex-col'>
  <View className='w-full h-fit flex-row justify-between items-center'>
  <Text className='text-[14px] font-medium py-4'>{item?.name || '음식 정보 없음'}</Text>
          ...
          >
            ...
            >
              {suggestions.map((suggestion, index) => (
                ...
                >
                  <Text className='text-[14px] text-gray-700'>{suggestion.name}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
          </View>
